...
import { FiPlusCircle, FiEdit, FiTrash2, FiLoader, FiX, FiSave, FiCheckCircle } from 'react-icons/fi';
...
// YENİ: Yeni/Düzenleme Formu için Modal Bileşeni
function SablonFormModal({
    ...
    const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
        ...
        startTransition(async () => {
            ...
            // TODO: Form içi başarı/hata mesajı göster
            if (result.success) {
                onClose();
            } else {
                alert(result.message); // Geçici hata gösterimi
            }
        });
    };
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                           <div>
                                <label htmlFor="gosterim_adi" className="block text-sm font-bold text-gray-600 mb-1">Gösterim Adı</label>
                                <input id="gosterim_adi" name="gosterim_adi" defaultValue={mevcutSablon?.gosterim_adi || ''} className="w-full p-2 border rounded-md" required />
                            </div>
                            <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select id="alan_tipi" name="alan_tipi" defaultValue={mevcutSablon?.alan_tipi || 'metin'} className="w-full p-2 border rounded-md bg-gray-50">
                                    ...
                                    <option value="boolean">Evet/Hayır (Checkbox)</option>
                                </select>
                            </div>
                            <div>
                        ...
                        <div className="pt-4 space-y-3">
                            ...
                            <div className="flex items-center">
                                <input type="checkbox" id="public_gorunur" name="public_gorunur" defaultChecked={mevcutSablon?.public_gorunur || false} className="h-4 w-4 rounded" />
                                <label htmlFor="public_gorunur" className="ml-2 text-sm text-gray-700">Public Portal'da Göster</label>
                            </div>
                            <div className="flex items-center">
                                <input type="checkbox" id="musteri_gorunur" name="musteri_gorunur" defaultChecked={mevcutSablon?.musteri_gorunur || false} className="h-4 w-4 rounded" />
                                <label htmlFor="musteri_gorunur" className="ml-2 text-sm text-gray-700">Müşteri Portal'ında Göster</label>
                            </div>
                            <div className="flex items-center">
                                <input type="checkbox" id="alt_bayi_gorunur" name="alt_bayi_gorunur" defaultChecked={mevcutSablon?.alt_bayi_gorunur || false} className="h-4 w-4 rounded" />
                                <label htmlFor="alt_bayi_gorunur" className="ml-2 text-sm text-gray-700">Alt Bayi Portal'ında Göster</label>
                            </div>
                        </div>
                    </div>
...
// ANA İSTEMCİ BİLEŞENİ (TAM VE GÜNCELLENMİŞ)
export function SablonYonetimIstemcisi({ serverKategoriler, locale }: SablonYonetimIstemcisiProps) {
  ...
  const handleSil = async (sablonId: string) => {
    if (confirm("Bu özelliği silmek istediğinizden emin misiniz? Bu işlem geri alınamaz ve bu özelliği kullanan ürünlerde veri kaybına yol açabilir.")) {
        await deleteSablonAction(sablonId);
    }
  }
                ...
                {sablonlar.map(sablon => (
                  <div key={sablon.id} className="p-4 bg-gray-50 border border-gray-200 rounded-md flex justify-between items-center hover:border-accent transition-all">
                    ...
                    <div className="flex gap-3">
                      ...
                      <button onClick={() => handleSil(sablon.id)} className="p-2 text-gray-500 hover:text-red-600 transition-colors" title="Sil"><FiTrash2 size={16} /></button>
                    </div>
                  </div>
                ))}
                {sablonlar.length === 0 && (
