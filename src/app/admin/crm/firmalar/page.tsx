// src/app/admin/crm/firmalar/page.tsx
...
export default async function FirmalarListPage({
    ...
    // Sorgu artık dinamik olarak filtrelere göre şekillenecek
    let query = supabase
        .from('firmalar')
        .select(`
    ...
    // Arama terimi varsa, 'unvan' sütununda (büyük/küçük harf duyarsız) ara
    if (searchQuery) {
        query = query.ilike('unvan', `%${searchQuery}%`);
    }

    // Statü filtresi varsa, 'status' sütununda tam eşleşme yap
    if (statusFilter) {
        query = query.eq('status', statusFilter);
    }
    
    // Son olarak sıralamayı ekle ve sorguyu çalıştır
    ...
    return (
        <div className="space-y-8">
            ...
            {firmaSayisi === 0 ? (
                ...
                <div>
                    {/* Mobil ve Tablet için Kart Görünümü */}
                    ...
                    {/* Geniş Ekranlar için Tablo Görünümü */}
