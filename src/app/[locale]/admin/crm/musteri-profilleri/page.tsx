...
export default async function CustomerProfilesPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  // Profil kullanım istatistikleri
  const { data: profilStats } = await (supabase as any)
    .from('firmalar')
    .select('musteri_profil_id, musteri_profilleri:musteri_profil_id(ad)')
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <h1 className="text-2xl font-semibold mb-6">Müşteri Profilleri</h1>
      ...
      <div className="bg-white rounded-lg border overflow-hidden">
        <table className="min-w-full">
          ...
          <tbody className="divide-y divide-gray-200">
            ...
            {((profiller ?? []) as any[]).map((profil: any) => (
              <tr key={profil.id}>
