// src/app/admin/crm/firmalar/yeni/page.tsx

import React from 'react';
import Link from 'next/link';
import { createSupabaseServerClient } from '@/lib/supabase/server';
import { redirect } from 'next/navigation';
// DÜZELTME 1: Gerekli olan 'TablesInsert' tipini de import ediyoruz.
...
async function yeniFirmaEkleAction(formData: FormData) {
  ...
  const sorumlu_ekip_uyesi_id = formData.get('sorumlu_ekip_uyesi_id') as string;
  ...
  // DÜZELTME 2: 'Tables' yerine 'TablesInsert' tipini kullanıyoruz.
  ...
  if (userRole === 'Ekip Üyesi') {
    insertData.sorumlu_ekip_uyesi_id = user.id;
  } else if (userRole === 'Yönetici' && sorumlu_ekip_uyesi_id) {
    insertData.sorumlu_ekip_uyesi_id = sorumlu_ekip_uyesi_id;
  }
...
// YENİ FİRMA EKLEME SAYFASI BİLEŞENİ (Bu kısımda değişiklik yok)
      ...
      <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-lg">
        <form action={yeniFirmaEkleAction}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                ...
                {userRole === 'Yönetici' ? (
                    <div>
                        <label htmlFor="sorumlu_ekip_uyesi_id" className="block text-sm font-bold text-text-main/80 mb-2">Sorumlu Personel (Opsiyonel)</label>
                        <select id="sorumlu_ekip_uyesi_id" name="sorumlu_ekip_uyesi_id" className={inputBaseClasses}>
