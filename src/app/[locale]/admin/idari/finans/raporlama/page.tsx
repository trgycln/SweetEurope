...
import { Enums, Database } from '@/lib/supabase/database.types'; // Database hinzugefügt
import { getDictionary } from '@/dictionaries';
import { Locale } from '@/i18n-config';
import { redirect } from 'next/navigation';

// ReportData Typ (Annahme: gider_kategorisi ist ein Enum, falls nicht, string verwenden)
type ReportData = {
    ...
    netProfit: number;
    expenseBreakdown: { kategori: string; toplam: number }[]; // Sicherer Typ: string
};
...
export default async function PNLReportPage({
    params: { locale },
    // searchParams ist KEIN Promise in Page Components
    searchParams
}: {
    ...
    const { data: report, error } = await supabase
        ...
        .returns<ReportData>() // Aktualisierten Typ verwenden
        .single();
            ...
            <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200">
                ...
                <div className="flex items-center gap-4 mb-6 border-b border-gray-200 pb-4">
                     ...
                     {datePeriods.map(p => (
                         <Link
                             key={p.value}
                             href={`/${locale}/admin/idari/finans/raporlama?period=${p.value}`}
                ...
                <div className="font-sans">
                     ...
                     {/* Verwende den sichereren 'string'-Typ für Kategorie */}
