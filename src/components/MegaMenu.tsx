...
const MegaMenu: React.FC<{ dictionary: any }> = ({ dictionary }) => {
  const { promo, mainCategories } = dictionary.megaMenu;
  // Üzerine gelinen ana kategoriyi takip etmek için bir state
  const [activeCategory, setActiveCategory] = useState<any | null>(null);

  return (
    <div 
      className="bg-secondary text-primary shadow-lg rounded-b-lg border-t border-gray-200"
      // Fare menünün tamamından ayrıldığında varsayılan promosyona geri dön
    ...
    >
      <div className="container mx-auto px-6 py-8 grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <nav className="flex flex-col space-y-2">
            {mainCategories.map((category: any) => (
              <div 
                ...
                className="flex justify-between items-center p-3 rounded-md hover:bg-bg-subtle cursor-pointer transition-colors"
              >
        ...
        {/* Sağ Sütun: Dinamik İçerik */}
        <div className="col-span-2">
          {/* Varsayılan Promosyon Görünümü */}
          {!activeCategory && (
             ...
             <div className="grid grid-cols-2 gap-8 h-full">
               <div>
                 ...
                 <nav className="flex flex-col space-y-3">
                  {activeCategory.subCategories.map((link: any) => (
