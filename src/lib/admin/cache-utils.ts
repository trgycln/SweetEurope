...
export const getCachedDashboardData = unstable_cache(
    async (period: string) => {
        ...
        const [
            ...
            supabase.from('firmalar').select('id', { count: 'exact' }).in('status', ['MÜŞTERİ', 'Müşteri', 'Alt Bayi']),
            ...
            supabase.from('firmalar').select('id', { count: 'exact' }).in('status', ['MÜŞTERİ', 'Alt Bayi']).gte('created_at', `${periodStart}T00:00:00`),
