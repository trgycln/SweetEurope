...
async function fixOncelikAndPuan() {
  console.log('🔧 Öncelik (oncelik) ve Puan (oncelik_puani) Sistemi Düzeltiliyor...\n');
  ...
  try {
    ...
    for (const firma of firmalar || []) {
      const dogruOncelik = firma.kategori; // Kategori = Oncelik
    ...
    for (const m of mismatches.slice(0, 10)) {
      ...
      console.log(`   Oncelik: ${m.oncekiOncelik} → ${m.dogruOncelik}`);
    ...
    for (const firma of updated || []) {
      const dogruOncelik = firma.kategori;
      const dogruPuan = hesaplaPuan(firma.kategori, firma.etiketler);
      ...
      if (isCorrect) {
        ...
        console.log(`⚠️  ${firma.unvan}: oncelik=${firma.oncelik} (beklenen: ${dogruOncelik}), puan=${firma.oncelik_puani} (beklenen: ${dogruPuan})`);
        allCorrect = false;
      }
    }
